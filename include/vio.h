...
public:
  ...
  std::shared_ptr<vk::PinholeCamera> pinhole_cam;
  StatesGroup *state;
  ...
  void getWarpMatrixAffine(const std::shared_ptr<vk::AbstractCamera> &cam, const Vector2d &px_ref, const Vector3d &f_ref, const double depth_ref, const SE3 &T_cur_ref,
                           const int level_ref, 
                           const int pyramid_level, const int halfpatch_size, Matrix2d &A_cur_ref);
  void getWarpMatrixAffineHomography(const std::shared_ptr<vk::AbstractCamera> &cam, const V2D &px_ref,
