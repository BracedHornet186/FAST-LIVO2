...
struct hash<VOXEL_LOCATION>
{
  int64_t operator()(const VOXEL_LOCATION &s) const
  {
    return ((((s.z) * VOXELMAP_HASH_P) % VOXELMAP_MAX_N + s.y)
             * VOXELMAP_HASH_P) % VOXELMAP_MAX_N + s.x;
  }
};
} // namespace std
...
public:
  ...
  float build_residual_time;
  float ekf_time;
  float ave_build_residual_time = 0.0f;
  float ave_ekf_time = 0.0f;
