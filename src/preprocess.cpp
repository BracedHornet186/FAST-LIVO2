...
static double time_to_sec(const builtin_interfaces::msg::Time& time) {
...
// NOTE: Depending on your livox driver version in ROS 2, this might be livox_ros_driver22::msg::CustomMsg
...
void Preprocess::l515_handler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
  ...
  // double time_stamp = time_to_sec(msg->header.stamp); // Unused variable
...
void Preprocess::oust64_handler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
  ...
  {
    // double time_stamp = time_to_sec(msg->header.stamp); // Unused
    for (int i = 0; i < pl_orig.points.size(); i++)
    {
      if (i % point_filter_num != 0) continue;
