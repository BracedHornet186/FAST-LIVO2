...
static double timeToSec(const builtin_interfaces::msg::Time& time) {
...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  const double &imu_beg_time = timeToSec(v_imu.front()->header.stamp);
  const double &imu_end_time = timeToSec(v_imu.back()->header.stamp);
  ...
  case VIO:
    ...
    for (int i = 0; i < v_imu.size() - 1; i++)
    {
      ...
      if (timeToSec(tail->header.stamp) < prop_beg_time) continue;
      ...
      fout_imu << setw(10) << timeToSec(head->header.stamp) - first_lidar_time << " " << angvel_avr.transpose() << " " << acc_avr.transpose() << endl;

      angvel_avr -= state_inout.bias_g;
      acc_avr = acc_avr * G_m_s2 / mean_acc.norm() - state_inout.bias_a;

      if (timeToSec(head->header.stamp) < prop_beg_time)
      {
        dt = timeToSec(tail->header.stamp) - last_prop_end_time;
        offs_t = timeToSec(tail->header.stamp) - prop_beg_time;
      }
      else if (i != v_imu.size() - 2)
      {
        dt = timeToSec(tail->header.stamp) - timeToSec(head->header.stamp);
        offs_t = timeToSec(tail->header.stamp) - prop_beg_time;
      }
      else
      {
        dt = prop_end_time - timeToSec(head->header.stamp);
