...
{
  // Helper lambda to declare and get parameters
  auto get_param = [&](std::string param_name, auto& param_var, auto default_val) {
      node->declare_parameter(param_name, default_val);
      param_var = node->get_parameter_or(param_name, param_var, param_var);
  };

  get_param("publish.pub_plane_en", voxel_config.is_pub_plane_map_, false);
  
  get_param("lio.max_layer", voxel_config.max_layer_, 1);
  get_param("lio.voxel_size", voxel_config.max_voxel_size_, 0.5);
  get_param("lio.min_eigen_value", voxel_config.planner_threshold_, 0.01);
  get_param("lio.sigma_num", voxel_config.sigma_num_, 3.0);
  get_param("lio.beam_err", voxel_config.beam_err_, 0.02);
  get_param("lio.dept_err", voxel_config.dept_err_, 0.05);

  // Handle vector<int> (ROS 2 uses vector<int64_t> for integer arrays)
  std::vector<int64_t> default_layer_init = {5, 5, 5, 5, 5};
  node->declare_parameter("lio.layer_init_num", default_layer_init);
  std::vector<int64_t> temp_layer_init = node->get_parameter("lio.layer_init_num").as_integer_array();
  voxel_config.layer_init_num_.assign(temp_layer_init.begin(), temp_layer_init.end());

  get_param("lio.max_points_num", voxel_config.max_points_num_, 50);
  get_param("lio.max_iterations", voxel_config.max_iterations_, 5);

  get_param("local_map.map_sliding_en", voxel_config.map_sliding_en, false);
  get_param("local_map.half_map_size", voxel_config.half_map_size, 100);
  get_param("local_map.sliding_thresh", voxel_config.sliding_thresh, 8.0);
}
